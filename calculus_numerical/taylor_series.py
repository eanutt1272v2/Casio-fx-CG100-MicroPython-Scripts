...
a_str=input("x range: a to "); a=float(a_str)
b=float(input("b: "))
...
def maclaurin(x, n, choice):
    s=0.0
    if choice=="1":
        for k in range(n): s+=(-1)**k*x**(2*k+1)/factorial(2*k+1)
    elif choice=="2":
        for k in range(n): s+=(-1)**k*x**(2*k)/factorial(2*k)
    elif choice=="3":
        for k in range(n): s+=x**k/factorial(k)
    elif choice=="4":
        for k in range(1,n+1): s+=(-1)**(k+1)*x**k/k
    elif choice=="5":
...
def _true_fn(choice, x):
    ...
    if choice=="3": return exp(x)
    if choice=="4": return log(1+x) if x>-1 else 0.0
    if choice=="5": return atan(x)
    return sin(x)
