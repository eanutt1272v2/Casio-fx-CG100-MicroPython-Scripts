RAM = {
    ...
    "Ag":107.9,"I":126.9,"Ba":137.3,"Pb":207.2,
}
...
def molar_mass(formula):
    ...
    for tok in _re.findall(r"([A-Z][a-z]?)(\d*)", formula):
        sym = tok[0]; cnt = int(tok[1]) if tok[1] else 1
...
while True:
    ...
    if m == "1":
        ...
        if not mass: mass=n*Mr
        if not Mr and n: Mr=mass/n
        if not n and Mr: n=mass/Mr
        ...
        if not c and n and V: c=n/V
        if not n and c and V: n=c*V
        if not V and c and n: V=n/c
        ...
        if not c1: c1=c2*V2/V1
        if not V1: V1=c2*V2/c1
        if not c2: c2=c1*V1/V2
        if not V2: V2=c1*V1/c2
        print("c1=",round(c1,5),"  V1=",round(V1,4),"cm3  c2=",round(c2,5),"  V2=",round(V2,4),"cm3")
    elif m == "6":
        f=input("Formula: "); Mr=molar_mass(f)
