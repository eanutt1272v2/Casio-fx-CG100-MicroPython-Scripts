...
def cmod(re,im): return math.sqrt(re*re+im*im)
def carg(re,im): return math.atan2(im,re)
def cmul(a,b,c,d): return (a*c-b*d, a*d+b*c)
def cdiv(a,b,c,d):
    denom=c*c+d*d
    return ((a*c+b*d)/denom,(b*c-a*d)/denom)
def cpow(r,im,n):
    mod=cmod(r,im); arg=carg(r,im)
    return (mod**n*math.cos(n*arg), mod**n*math.sin(n*arg))
def fmt(x): return round(x,5)
...
while True:
    ...
    if m=="1":
        ...
        r=cmod(a,b); theta=carg(a,b)
        print("Mod=",fmt(r),"Arg=",fmt((theta)*180/math.pi),"deg")
        print("Polar: "+str(fmt(r))+"(cos"+str(fmt((theta)*180/math.pi))+"+ i sin...)")
        conj=(a,-b); print("Conjugate=",a,"-",abs(b),"i")
        plot([0,a],[0,b],"blue"); scatter([a],[b])
        plot([-2,2],[0,0],"grey"); plot([0,0],[-2,2],"grey")
        text(a,b,"z"); axis([-r*1.5,r*1.5,-r*1.5,r*1.5]); show()
    elif m=="2":
        raw=input("z1 = a,b: ").split(","); a,b=float(raw[0]),float(raw[1])
        raw=input("z2 = c,d: ").split(","); c,d=float(raw[0]),float(raw[1])
        re,im=cmul(a,b,c,d)
        ...
        raw=input("z = a,b: ").split(","); a,b=float(raw[0]),float(raw[1])
        n=int(input("n: "))
        re,im=cpow(a,b,n)
