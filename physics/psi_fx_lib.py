from math import sqrt, log, exp, pi, sin, cos, acos, atan2
...
UNIT_LABELS = {1: "[a0^-3] (Atomic)", 2: "[m^-3] (SI Metric)"}
...
def lgamma(x):
    if x < 0.5:
        return log(pi / (sin(pi * x) + EPSILON)) - lgamma(1.0 - x)
    x -= 1.0
...
def get_R(n, l, Z, offset, k_scale):
    inner_term = max(0, n * n - l * (l + 1))
    r_turn = (n * n + n * sqrt(inner_term)) / Z
    R = r_turn * k_scale
    if abs(offset) > 0.0:
        R = sqrt(R * R + offset * offset)
    return R


def radial_density_sq(n, l, x):
    if x < 0:
        return 0.0
    val = exp(-0.5 * x) * (x**l) * _lag(n - l - 1, 2 * l + 1, x)
    return val * val


def angular_density_sq(l, m, u):
    val = _al(l, abs(m), max(-1.0, min(1.0, u)))
    return val * val


def get_peak_density(n, l, m, Z, density_3d_local):
    steps_r = 1024
    b_r = 2.0 * n * (n + 1.0)
    grid_r = [b_r * i / steps_r for i in range(steps_r + 1)]
    best_i_r = 0
    max_rv = -1.0
    for i in range(len(grid_r)):
        x_val = grid_r[i]
        v = radial_density_sq(n, l, x_val)
        if v > max_rv:
            max_rv = v
            best_i_r = i
    a = grid_r[max(0, best_i_r - 1)]
    b = grid_r[min(steps_r, best_i_r + 1)]
    for _ in range(64):
        c = a + (b - a) * 0.38197
        d = b - (b - a) * 0.38197
        if radial_density_sq(n, l, c) < radial_density_sq(n, l, d):
            a = c
        else:
            b = d
    x_max = (a + b) / 2.0
    steps_u = 512
    grid_u = [-1.0 + 2.0 * i / steps_u for i in range(steps_u + 1)]
    best_i_u = 0
    max_uv = -1.0
    for i in range(len(grid_u)):
        u_val = grid_u[i]
        v = angular_density_sq(l, m, u_val)
        if v > max_uv:
            max_uv = v
            best_i_u = i
    a = grid_u[max(0, best_i_u - 1)]
    b = grid_u[min(steps_u, best_i_u + 1)]
    for _ in range(64):
        c = a + (b - a) * 0.38197
        d = b - (b - a) * 0.38197
        if angular_density_sq(l, m, c) < angular_density_sq(l, m, d):
            a = c
        else:
            b = d
    u_max = (a + b) / 2.0
    r_peak = x_max * n / (2.0 * Z)
    th_peak = acos(max(-1.0, min(1.0, u_max)))
    peak = density_3d_local(r_peak * sin(th_peak), 0.0, r_peak * cos(th_peak))
    return max(peak, 1e-30)


def format_hdr(n, l, m, Z, basis, plane, offset, cm_str, R, k, phi):
    Zs = int(Z)
    if Z != Zs:
        Zs = Z 
    return "|%s,%s,%s> Z=%s offset=%.1f phi=%.1f R=%.1f k=%.1f %s %s %s" % (
        n, l, m, Zs, offset, phi, R, k, plane, basis, cm_str
    )


def get_unit_map(wf):
    return {1: (1.0, "[a0^-3]"), 2: (wf.a_mu * wf.a_mu * wf.a_mu, "[m^-3]")}


def generate_colour_lut(cmap_idx, registry):
...
class HydrogenicWavefunction:
    def __init__(self, n, l, m, Z, M_kg, use_reduced_mass, phi_slice, plane_choice, offset, is_real=False):
        ...
        mu = (M_E * M_P) / (M_E + M_P) if (use_reduced_mass and M_kg is None) else (M_E * M_kg / (M_E + M_kg) if use_reduced_mass else M_E)
        self.a_mu = A0 * (M_E / mu)
        self.p_rad, self.alpha_l = n - l - 1, 2 * l + 1
        self.rho_k_grid = 2.0 * Z / n
        self.norm_r = exp(1.5 * log(self.rho_k_grid) + 0.5 * (lgamma(n - l) - log(2.0 * n) - lgamma(n + l + 1)))
        log_norm_y = 0.5 * (log((2 * l + 1) / (4 * pi)) + lgamma(l - self.abs_m + 1) - lgamma(l + self.abs_m + 1))
        if self.is_real and m != 0:
            log_norm_y += 0.5 * log(2.0)
        self.y_norm = exp(log_norm_y)
    ...
    def density_3d(self, x, y, z):
        r2 = x * x + y * y + z * z
        if r2 <= 1e-24:
            return (
                (self.norm_r * _lag(self.p_rad, self.alpha_l, 0.0) * self.y_norm * _al(self.l, self.abs_m, 1.0)) ** 2
                if self.l == 0 and self.abs_m == 0
                else 0.0
            )
        r_grid = sqrt(r2)
        rho = self.rho_k_grid * r_grid
        ea = -0.5 * rho + (self.l * log(rho) if self.l > 0 else 0.0)
        if ea < -700.0:
            return 0.0
        rv = self.norm_r * exp(ea) * _lag(self.p_rad, self.alpha_l, rho)
        yv = self.y_norm * _al(self.l, self.abs_m, max(-1.0, min(1.0, z / (r_grid + EPSILON))))
        if self.is_real:
            phase = 1.0 if self.m == 0 else (cos(self.m * atan2(y, x)) if self.m > 0 else sin(self.abs_m * atan2(y, x)))
            return rv * yv * phase * rv * yv * phase
        return rv * rv * yv * yv
